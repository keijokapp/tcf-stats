...
import { extractPeriod, getDiff, getTable, logger, reqid, skillId, worldId } from './common';
...
app.use((req, res) => {
	...
	if('world' in metadata) {
		...
		Promise.all([getTable(world, skill, period.startTime), getTable(world, skill, period.endTime)])
			.then(([startTable, endTable]) => {
				res.send(render(metadata, period, startTable, endTable));
			}, e => {
