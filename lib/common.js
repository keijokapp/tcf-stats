 ...
 * @returns {number}
 */
export function calculateFame(skills) {
	let fame = 0;
 ...
 * @returns {Promise}
 */
export function timeout(period) {
...
async function getTables(...descriptors) {
	const keys = [];
	for(const { world, skill, date } of descriptors) {
...
function processTables(startTable, endTable) {
	...
	for(const i in endTable.users) {
		users[i] = { user: i, rank: endTable.users[i].rank, value: endTable.users[i].value };
	...
	return { users, newUsers, changedUsers };
}
...
// very naive cache clearing implementation
// empties cache approximately every 1 hour
setInterval(() => {
	logger.info('Clearing cache', { cacheSize: Object.keys(statsCache).length });
	statsCache = {};
}, 60 * 3600 * 1000);
 ...
 */
export async function getStats(world, skill, period) {
	const cacheKey = getCacheKey(world, skill, period);

	if(cacheKey in statsCache) {
		logger.debug('Cache hit', { world, skill, startTime: period.startTime, endTime: period.endTime });
		return statsCache[cacheKey];
	}

	logger.debug('Cache miss', { world, skill, startTime: period.startTime, endTime: period.endTime });

	const [startTable, endTable] =
		await getTables({ world, skill, date: period.startTime }, { world, skill, date: period.endTime });
...
async function fetchTablePage(world, skill, page) {
	const response = await fetch(`https://crime.ee/index.php?a=8&top=${skill}&m=${world}`, {
		method: 'POST',
		headers: {
 ...
 * @param skill {integer} numeric skill id
 */
export async function createTable(world, skill) {
	let users = await fetchTable(world, skill);
	let date;
	logger.debug('Fetching table', { world, skill });
	confirm: while(true) {
		date = new Date;
		logger.debug('Confirming', { world, skill, date });
	...
	logger.info('Fetched table', { world, skill, date, size: Object.keys(users).length });

	const docId = `snapshot/${world}-${skill}-${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`;

	try {
		await db.put({ _id: docId, world, skill, time: date.getTime(), users });
	} catch(e) {
		if(e.name === 'conflict') {
			logger.warn('Snapshot already exists', { world, skill, date });
		} else {
			throw e;
		}
	}
