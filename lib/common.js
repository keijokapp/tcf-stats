...
async function createTable(world, skill) {
	...
	confirm: while(true) {
		date = new Date;
		console.log('Confirming', { world, skill, date });
	...
	console.log('Fetched table', { world, skill, date, size: Object.keys(users).length });

	const docId = `snapshot/${world}-${skill}-${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`;
	// await db.post({ _id: docId, world, skill, time, users });
}
