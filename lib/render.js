		...
		for(let i = columnIndex * numberOfRows; i < columnIndex * numberOfRows + numberOfRows; i++) {
			...
			if(user) {
				rows.push(<tr key={i}>
					...
					<td><a
						href={`https://www.crime.ee/index.php?a=11&m=${world}&k=${encodeURIComponent(user.user)}`}
		...
		rows.push(<tr key={i}>
			...
			<td className="number"
			    style={{ color: user.normalizedRankDiff > 0 ? 'green'
					    : (user.normalizedRankDiff < 0 ? 'red' : undefined) }}>
				<b>{user.normalizedRankDiff}</b> ({user.rankDiff})</td>
			<td className="number"><b>{user.valueDiff.toFixed(props.metadata.skill === 'kaklemine' ? 2 : 0)}</b></td>
...
function Index(props) {
	...
	<head>
		...
		<link rel="icon" href="https://crime.ee/favicon.ico"/>
		<link rel="stylesheet" href="https://valge.crime.ee/css/layout/wide.css?2.7"/>
		<link rel="stylesheet" href="https://valge.crime.ee/css/colors/crime.css?2.7"/>
		<link rel="stylesheet" href="https://valge.crime.ee/css/fonts/default.css?2.7"/>
		<style>{`
			...
			background-image: url(https://crime.ee/pic/cont-head-gradient.png);
