...
function SkillDiff(props) {
	...
	for(let i = 0; i < props.users.length; i++) {
		...
		rows.push(<tr key={i}>
			...
			<td className="number"><b>{calculateFame({ [props.metadata.skill]: user.value}) - calculateFame({ [props.metadata.skill]: user.value - user.valueDiff })}</b></td>
		</tr>);
	}
